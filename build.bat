...
if not exist "%RAYLIB_LIB%" (
    ...
    gcc -c "%RAYLIB%\rshapes.c" -o "%RAYLIB%\rshapes_win.obj" -O2 -I"%RAYLIB%" -std=c11
    if errorlevel 1 exit /b %errorlevel%

    gcc -c "%RAYLIB%\rtextures.c" -o "%RAYLIB%\rtextures_win.obj" -O2 -I"%RAYLIB%" -std=c11
    if errorlevel 1 exit /b %errorlevel%

    gcc -c "%RAYLIB%\rtext.c" -o "%RAYLIB%\rtext_win.obj" -O2 -I"%RAYLIB%" -std=c11
    if errorlevel 1 exit /b %errorlevel%

    gcc -c "%RAYLIB%\rmodels.c" -o "%RAYLIB%\rmodels_win.obj" -O2 -I"%RAYLIB%" -std=c11
    if errorlevel 1 exit /b %errorlevel%

    gcc -c "%RAYLIB%\utils.c" -o "%RAYLIB%\utils_win.obj" -O2 -I"%RAYLIB%" -std=c11
    if errorlevel 1 exit /b %errorlevel%
