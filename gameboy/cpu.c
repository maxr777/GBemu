...
bool
get_flag(const CPU *cpu, const int flag) {
	switch (flag) {
	case Z:
...
void
set_flag(CPU *cpu, const int flag, const bool value) {
	switch (flag) {
	case Z:
		if (value)
...
void
ld_r8_r8(Gameboy *gb, u8 *dest, const u8 src) {
	*dest = src;
...
void
ld_r8_n8(Gameboy *gb, u8 *dest, const u8 val) {
	*dest = val;
...
void
ld_r16_n16(Gameboy *gb, u16 *dest, const u16 val) {
	*dest = val;
...
void
ld_aHL_r8(Gameboy *gb, const u8 src) {
...
void
ld_aHL_n8(Gameboy *gb, const u8 val) {
...
void
ld_a16_A(Gameboy *gb, const u16 addr) {
...
void
ld_addr16_A(Gameboy *gb, const u16 addr) {
	write8(gb, addr, gb->cpu.regs[AF].high);
...
void
ldh_addr16_A(Gameboy *gb, const u16 addr) {
	write8(gb, addr, gb->cpu.regs[AF].high);
...
void
ld_A_addr16(Gameboy *gb, u16 addr) {
	gb->cpu.regs[AF].high = read8(gb, addr);
...
void
ldh_A_addr16(Gameboy *gb, const u16 addr) {
	gb->cpu.regs[AF].high = read8(gb, addr);
...
void
add_A_r8(Gameboy *gb, const u8 src) {
...
void
add_A_n8(Gameboy *gb, const u8 val) {
	set_flag(&gb->cpu, N, false);
	set_flag(&gb->cpu, H, (gb->cpu.regs[AF].high & 0x0F) + (val & 0x0F) > 0x0F);
	set_flag(&gb->cpu, C, (gb->cpu.regs[AF].high + val) > 0xFF);
...
void
adc_A_r8(Gameboy *gb, const u8 src) {
	set_flag(&gb->cpu, N, false);
...
void
adc_A_n8(Gameboy *gb, const u8 val) {
	set_flag(&gb->cpu, N, false);
	bool c = get_flag(&gb->cpu, C);
...
void
sub_A_r8(Gameboy *gb, const u8 src) {
...
void
sub_A_n8(Gameboy *gb, const u8 val) {
	set_flag(&gb->cpu, N, true);
	set_flag(&gb->cpu, H, (val & 0x0F) > (gb->cpu.regs[AF].high & 0x0F));
	set_flag(&gb->cpu, C, val > gb->cpu.regs[AF].high);
...
void
sbc_A_r8(Gameboy *gb, const u8 src) {
	set_flag(&gb->cpu, N, true);
	bool c = get_flag(&gb->cpu, C);
...
void
sbc_A_n8(Gameboy *gb, const u8 val) {
	set_flag(&gb->cpu, N, true);
	bool c = get_flag(&gb->cpu, C);
...
dec_aHL(Gameboy *gb) {
	...
	write8(gb, gb->cpu.regs[HL].full, result);
...
void
cp_A_r8(Gameboy *gb, const u8 src) {
...
void
cp_A_n8(Gameboy *gb, const u8 val) {
	u8 result = gb->cpu.regs[AF].high - val;
...
void
add_HL_r16(Gameboy *gb, const u16 src) {
	set_flag(&gb->cpu, N, false);
...
void
and_A_r8(Gameboy *gb, const u8 src) {
	gb->cpu.regs[AF].high &= src;
...
void
and_A_n8(Gameboy *gb, const u8 val) {
	gb->cpu.regs[AF].high &= val;
...
void
or_A_r8(Gameboy *gb, const u8 src) {
	gb->cpu.regs[AF].high |= src;
...
void
or_A_n8(Gameboy *gb, const u8 val) {
	gb->cpu.regs[AF].high |= val;
...
void
xor_A_r8(Gameboy *gb, const u8 src) {
	gb->cpu.regs[AF].high ^= src;
...
void
xor_A_n8(Gameboy *gb, const u8 val) {
	gb->cpu.regs[AF].high ^= val;
...
void
bit_u3_r8(Gameboy *gb, const int bit_num, const u8 src) {
...
void
bit_u3_aHL(Gameboy *gb, const int bit_num) {
...
void
res_u3_r8(Gameboy *gb, const int bit_num, u8 *src) {
	*src &= ~(1 << bit_num);
...
void
res_u3_aHL(Gameboy *gb, const int bit_num) {
	u8 val = read8(gb, gb->cpu.regs[HL].full);
	val &= ~(1 << bit_num);
	write8(gb, gb->cpu.regs[HL].full, val);
...
void
set_u3_r8(Gameboy *gb, const int bit_num, u8 *src) {
	*src |= (1 << bit_num);
...
void
set_u3_aHL(Gameboy *gb, const int bit_num) {
	u8 val = read8(gb, gb->cpu.regs[HL].full);
	val |= (1 << bit_num);
	write8(gb, gb->cpu.regs[HL].full, val);
...
void
call_n16(Gameboy *gb, const u16 addr) {
...
void
call_cc_n16(Gameboy *gb, const int flag, const bool flag_state, const u16 addr) {
...
void
jp_n16(Gameboy *gb, const u16 addr) {
	gb->cpu.regs[PC].full = addr;
...
void
jp_cc_n16(Gameboy *gb, const int flag, const bool flag_state, const u16 addr) {
	if (get_flag(&gb->cpu, flag) == flag_state) {
		gb->cpu.regs[PC].full = addr;
		gb->cpu.cycle += 4;
	} else {
...
void
jr_n16(Gameboy *gb, const i8 offset) {
	gb->cpu.regs[PC].full += 2;
...
// It's n16 because it jumps to address n16, but it uses an 8-bit offset instead.
void
jr_cc_n16(Gameboy *gb, const int flag, const bool flag_state, const i8 offset) {
	gb->cpu.regs[PC].full += 2;
	if (get_flag(&gb->cpu, flag) == flag_state) {
		gb->cpu.regs[PC].full += offset;
		gb->cpu.cycle += 3;
...
void
ret_cc(Gameboy *gb, const int flag, const bool flag_state) {
	if (get_flag(&gb->cpu, flag) == flag_state) {
		gb->cpu.regs[PC].low = read8(gb, gb->cpu.regs[SP].full);
		++gb->cpu.regs[SP].full;
		gb->cpu.regs[PC].high = read8(gb, gb->cpu.regs[SP].full);
...
void
push_r16(Gameboy *gb, const u16 src) {
	gb->cpu.regs[SP].full -= 2;
...
void
ld_addr16_SP(Gameboy *gb, const u16 addr) {
...
void
ld_HL_SPe8(Gameboy *gb, const i8 val) {
	set_flag(&gb->cpu, Z, false);
	set_flag(&gb->cpu, N, false);
...
void
add_SP_n8(Gameboy *gb, const i8 val) {
	set_flag(&gb->cpu, Z, false);
	set_flag(&gb->cpu, N, false);
	set_flag(&gb->cpu, H, (gb->cpu.regs[SP].full & 0x000F) + (val & 0x0F) > 0x000F);
	set_flag(&gb->cpu, C, (gb->cpu.regs[SP].full & 0x00FF) + (val & 0xFF) > 0x00FF);
...
void
stop_n8(Gameboy *gb, const u8 val) {
