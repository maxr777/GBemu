...
static void check_game_load(const char *filepath, const ROM *expected) {
	...
	assert(rom.game_rom[0x014F] == 0x55);
...
static void test_game_load(void) {
	...
	memcpy(expected.cartridge_header.nintendo_logo,
	       (u8[]){
		   0xCE,
		   0xED,
		   0x66,
	       ...
	       sizeof(expected.cartridge_header.nintendo_logo));
	...
	expected.cartridge_header.global_checksum = 0x551C;
